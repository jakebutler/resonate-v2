...
describe("v2 domain helpers", () => {
  ...
  it("asks clarifying questions for thin multi-channel Ideas", () => {
    const idea = {
      ...DEFAULT_WORKSPACE_STATE.ideas[0],
      sourceUrl: undefined,
      tags: [],
      entries: [
        {
    ...
    expect(
      buildClarifyingQuestions({
        idea,
        brand: BRANDS.find((brand) => brand.id === "corvo")!,
  ...
  it("creates a Corvo blog draft with source material and voice-pack context", () => {
    ...
    const draft = buildCorvoBlogDraft({
      idea,
      voicePackMarkdown: DEFAULT_WORKSPACE_STATE.voicePacks[0].markdown,
    });
  ...
  describe("buildFallbackDraft", () => {
    const idea = DEFAULT_WORKSPACE_STATE.ideas[0];
    const voicePackMarkdown = DEFAULT_WORKSPACE_STATE.voicePacks[0].markdown;
  ...
  describe("filterPostsForView", () => {
    const makePost = (id: string, brandId: Post["brandId"], status: Post["status"]): Post => ({
      id,
      brandId,
      channelId: "linkedin",
      title: `Post ${id}`,
  ...
  it("GapSeverity type covers expected severity levels", () => {
    // Type-level smoke test — ensures the union is exportable and assignable
    const severities: GapSeverity[] = ["blocker", "v1.1", "later", "acceptable"];
    expect(severities).toHaveLength(4);
  });

  describe("brand/channel/provider routing", () => {
    it("creates all MVP brand workspaces and routable validation channels", () => {
      expect(CHANNELS.some((c) => c.brandId === "personal")).toBe(true);
      expect(CHANNELS.some((c) => c.brandId === "corvo")).toBe(true);
      expect(CHANNELS.some((c) => c.brandId === "lower-db")).toBe(true);
      expect(CHANNELS.some((c) => c.brandId === "freshproof")).toBe(true);
      expect(findChannel("corvo", "linkedin")?.providerId).toBe("buffer");
      expect(findChannel("freshproof", "reddit")?.providerId).toBe("zernio");
      expect(findChannel("corvo", "corvo-blog")?.providerId).toBe("github-pr");
      expect(findChannel("corvo", "x")?.routable).toBe(false);
    });
  });
