...
describe("research editorial pipeline types and helpers", () => {
  ...
  describe("SourceRecord status transitions", () => {
    it("a source starts as unvetted and can be accepted", () => {
      ...
      const accepted: SourceRecord = { ...record, status: "accepted" };
      expect(accepted.status).toBe("accepted");

      const rejected: SourceRecord = { ...record, status: "rejected" };
      expect(rejected.status).toBe("rejected");

      const flagged: SourceRecord = { ...record, status: "flagged" };
