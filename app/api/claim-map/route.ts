...
function buildMockClaims(sources: SourceRecord[]) {
    ...
    const claims = rawClaims
      ...
      .map((c) =>
        makeClaim({
          ...
          evidenceLabel: (c.evidenceLabel as EvidenceLabel) ?? "weaker-support",
          confidence: (c.confidence as ClaimConfidence) ?? "medium",
          caveats: c.caveats,
        })
      );
