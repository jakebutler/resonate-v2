...
| #3 Glossary and ADRs | `docs/glossary.md`, `docs/adr/0001-postiz-spine.md`, `docs/adr/0002-provider-routing-and-approval.md`, `docs/adr/0003-deployment-runtime.md`, `docs/adr/0004-single-composer.md` | Keep glossary current as provider/runtime choices harden |
...
| #7 Single composer | persisted v2 composer shell in `PersistedPublishingPanel` (canonical surface per ADR 0004); research/AI tracer moved to sibling `/v2/research` route; legacy editors at `/app/editor/[id]` remain v1-parity read-only until cutover | Add per-platform settings (LinkedIn CTA/hashtags, Reddit subreddit/flair, Corvo Blog metadata) in B.2.A; redirect v2 posts opened via `/editor/[id]` to v2 composer |
...
- `gh repo view` confirms the accessible Corvo Blog repository is `jakebutler/corvo-labs-dot-com` with default branch `main`.
