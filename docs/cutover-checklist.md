...
Status: **NOT READY — active rework toward reviewable parity**

> **2026-06-06 status correction.** A prior pass marked this "CUTOVER COMPLETE / GO."
> That overstated readiness. A scrutiny audit found a split-brain persistence model
> (the `/v2/research` idea inbox runs on browser `localStorage`, not Convex), provisioning
> jargon in the UI ("Seed Workspace", "Persisted MVP Spine"), an illegible Blog PR authoring
> flow, and social posting that is simulated-only. v2 is **not** in a state where the core
> workflows can be fairly evaluated against v1. See ADR 0005 and the reviewable-parity PRD
> (issue #1 thread) for the rework plan. Do not treat this checklist's GO as current.
...
## 4. Deployment URLs and Smoke Testing


| URL                                     | Project                   | Role                                                                  |
| --------------------------------------- | ------------------------- | --------------------------------------------------------------------- |
| `**https://resonate.corvolabs.com/v2`** | `resonate-v2` (this repo) | **Production — use for C.3 final smoke**                              |
| `https://resonate-v2-delta.vercel.app`  | `resonate-v2`             | Default Vercel hostname; Clerk auth may fail without satellite config |
| Legacy v1                               | `resonate` Vercel project | Deprecated after C.3 sign-off; archive Convex first                   |


Cutover alias completed 2026-06-06: `resonate.corvolabs.com` → resonate-v2 production deployment.

## 4b. Operating Model (post-cutover)


| Workflow                             | Owner system             | Notes                                                                 |
| ------------------------------------ | ------------------------ | --------------------------------------------------------------------- |
| New idea capture                     | **V2**                   | `/v2/research`                                                        |
| Draft generation (AI)                | **V2**                   | Multi-channel variants via V2                                         |
| Source research (FreshProof)         | **V2 Research Pipeline** | Only available in V2                                                  |
| Post scheduling                      | **V2**                   | Calendar + approval + provider state; live Buffer/Zernio gated by env |
| Corvo Blog PR                        | **V2**                   | /api/publish already wired                                            |
| Social publishing (LinkedIn, Reddit) | **V2 adapters**          | Buffer + Zernio live when `*_LIVE_SUBMISSION=approved`                |
| Analytics                            | **Not in V2**            | Remains out of scope for MVP                                          |
...
**Recommendation (superseded 2026-06-06): NO-GO — v2 is in active rework toward reviewable parity.**

The prior "GO" below is retained for history only. The reviewable-parity rework (Convex as
single source of truth, legible onboarding and Blog PR authoring, plain-language UI, validated
social authentication with simulated posting, real v1 idea import) must land and pass a written
evaluation script before any cutover recommendation is reconsidered.

<details>
<summary>Superseded prior recommendation</summary>
